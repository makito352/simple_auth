...
- **WebAuthn (FIDO2) による多要素認証**: パスワードの代わりに生体認証や物理キーを用いることで、フィッシング攻撃やパスワード漏洩のリスクを根本から排除します。
...
1. コンテナ起動後、以下のコマンドでログを確認します。
   ...
   ```
2. `Created initial admin user with email: ...` というメッセージの後に表示される **one-time link** をブラウザで開き、初期設定を完了させてください。
### 3. 外部サービス連携（オプション）
#### Photoprism を連携する場合
...
## 🚀 Roadmap (今後の展望)
- [ ] 管理者用ダッシュボードのUI/UX改善
- [ ] 複数ドメイン間でのOIDC連携支援機能の強化
- [ ] より高度な権限管理（RBAC）の実装

## ロゴの画像ファイルについて
Copilot生成素材を使用しております。
