...
def list_credentials(
    ...
    try:
        ...
        # 例外が発生した場合のエラーログを出力し、HTTP 500エラーを返す
        logger.error(f"Error while listing credentials: {e}", exc_info=True)
        raise HTTPException(
...
def update_credential_comment(
    ...
    try:
        ...
        # 例外が発生した場合のエラーログを出力し、HTTP 500エラーを返す
        logger.error(f"Error while updating credential comment: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal server error",
        )
...
def delete_credential(
    ...
    try:
        ...
        # 例外が発生した場合のエラーログを出力し、HTTP 500エラーを返す
        logger.error(f"Error while deleting credential: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal server error",
        )
