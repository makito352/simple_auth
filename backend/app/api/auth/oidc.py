...
def _raise_internal_server_error(
    ...
    **context,
    ...
    """
    OIDCエンドポイント内での想定外例外を統一的に記録し、
    外部には共通の500エラーだけを返す。

    Args:
        endpoint (str): 例外が発生したエンドポイント名
        error (Exception): 発生した例外
        **context: ログに残す補足コンテキスト
    """
    context_text = ", ".join(
...
def openid_configuration(db: Session = Depends(get_db)) -> OpenIdConfigurationResponse:
    ...
    try:
        ...
        scope_names = [
            scope.scope_name for scope in OidcClientService.get_all_scopes(db)
        ]
        ...
        _raise_internal_server_error("openid_configuration", error)
...
def jwks() -> JwksResponse:
    ...
    try:
        ...
        _raise_internal_server_error("jwks", error)
...
def authorize(
    ...
    try:
        ...
        try:
            ...
            # 認可画面で未ログインの場合、401ではなくログイン画面へリダイレクトさせたい場合は
...
def userinfo(request: Request, db: Session = Depends(get_db)) -> UserInfoResponse:
    try:
        ...
        _raise_internal_server_error("userinfo", error)
