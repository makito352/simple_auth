/**
 * @file frontend/src/components/features/admin/oidc/OidcManagementPage.tsx
...
import type { ClaimMapping, ClaimMappingInput, OidcScope, OptionAttribute } from "@/types";
...
export default function OidcManagementPage() {
   ...
   * @param scopeOptions 利用可能なスコープのリスト
   */
              ...
              <tbody>
                {mappings.map((mapping) => (
                  <tr key={mapping.id} className="hover:bg-gray-50">
                    <td className="p-3 border-b">{mapping.scope}</td>
                    <td className="p-3 border-b">{mapping.claim_name}</td>
                    <td className="p-3 border-b">{getValueSourceLabel(mapping.value_source)}</td>
                    <td className="p-3 border-b">
                      {mapping.value_source === "static" ? `Val: ${mapping.static_value}` : `Key: ${mapping.value_key}`}
                    </td>
                    <td className="p-3 border-b space-x-2">
                      <button onClick={() => handleEdit(mapping)} className="text-blue-600"><Edit2 size={18} /></button>
                      <button onClick={() => handleDelete(mapping.id)} className="text-red-600"><Trash2 size={18} /></button>
                    </td>
                  </tr>
                ))}
                {mappings.length === 0 && (
